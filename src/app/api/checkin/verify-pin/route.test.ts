...
vi.mock("@/lib/registrations-store", () => ({ getPaidAttendees: vi.fn() }));
...
function request(body: unknown): Request {
  return new Request("http://localhost/api/checkin/verify-pin", {
    method: "POST",
...
describe("POST /api/checkin/verify-pin", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    vi.mocked(getPaidAttendees).mockResolvedValue([]);
  });
  ...
  it("skips PIN verification and rate limiting entirely when an existing valid session is present", async () => {
    ...
    mockCookie(real);
    const res = await POST(request({})); // no pin in body at all
    expect(res.status).toBe(200);
    expect(isLockedOut).not.toHaveBeenCalled();
    expect(getPaidAttendees).toHaveBeenCalledWith(EVENT.id);
  });

  it("rejects the correct PIN if presented via a session token forged under the wrong secret — the token, not the PIN, gates this shortcut", async () => {
    ...
    // Falls through to the normal PIN-check path since the forged token doesn't verify — not an
    // error by itself, since the real PIN is supplied and should still succeed via that path.
    expect(res.status).toBe(200);
  });
  ...
  it("returns 429 once locked out, without checking the PIN", async () => {
    ...
    expect(getPaidAttendees).not.toHaveBeenCalled();
  });
  ...
  it("accepts the correct PIN and issues a session cookie that is httpOnly and sameSite=strict", async () => {
    ...
    expect(setCookie).toContain(`${CHECKIN_SESSION_COOKIE}=`);
  ...
  it("tolerates a malformed JSON body without throwing", async () => {
    mockCookie(undefined);
    const malformed = new Request("http://localhost/api/checkin/verify-pin", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "not json",
    });
