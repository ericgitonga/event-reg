...
vi.mock("@/lib/registrations-store", () => ({ markPaid: vi.fn() }));
...
function request(body: unknown): Request {
  return new Request("http://localhost/api/payments/mark", {
    method: "POST",
...
describe("POST /api/payments/mark", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("accepts a real session token and marks the registration paid", async () => {
    const real = createOrganiserSessionToken(EVENT.id, EVENT.sessionSecret);
    mockCookie(real);
    vi.mocked(markPaid).mockResolvedValue(true);
    const res = await POST(request({ registrationId: "reg-1" }));
